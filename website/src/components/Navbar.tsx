...
import { IconTerminal, IconCopy, IconCheck, IconExternalLink, IconBrandGithub } from '@tabler/icons-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <a href="#hero" className="flex items-center gap-3 group">
          <div className="w-8 h-8 rounded-lg bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 group-hover:border-emerald-400 transition p-1">
            <img src="/logo.png" alt="Purgo Logo" className="w-full h-full object-contain" />
          </div>
        ...
        <div className="hidden lg:flex items-center gap-6 text-xs font-sans font-medium text-slate-300">
          ...
          <a href="#simulator" className="hover:text-emerald-400 transition">Simulator</a>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            onClick={handleCopy}
            className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-mono bg-slate-900 hover:bg-slate-800 text-slate-300 border border-[#1e2638] hover:border-emerald-400/40 transition"
          >
            <span className="text-emerald-400 font-bold">$</span>
            <span>purgo --clean</span>
            {copied ? <IconCheck size={14} stroke={2} className="text-emerald-400 ml-1" /> : <IconCopy size={14} stroke={1.75} className="text-slate-400 ml-1" />}
          </button>
